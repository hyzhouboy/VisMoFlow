...
if __name__ == '__main__':
    ...
    name = 'image_2_events'
    dir_ = 'F:/Research/Experiment_Code/2024CVPR/Dataset/KITTI/Daytime/training'
    path = os.path.join(dir_, name)

    
    time_delta = 15000
    frame_id = 0
    files = sorted(os.listdir(os.path.join(path, 'timestamp')))
    # 创建文件夹
    ...
    for index in range(len(files)):
        print(files[index])
        ev_data_file = os.path.join(path+'/events', files[index][:-4] + '.h5')
        ev_location = h5py.File(str(ev_data_file), 'r')
        events_slicer = EventSlicer_KITTI(ev_location)
        with open(os.path.join(path+'/timestamp/', files[index]), 'r', encoding="utf-8") as file:
            lines = file.readlines()
            # for index in range(len(files)):
            t1 = int(lines[10].split(' ')[1].split('\n')[0].split('.')[0])
            t2 = int(lines[11].split(' ')[1].split('\n')[0].split('.')[0])

            events = events_slicer.get_events(t1, t1+time_delta)
            ...
            save_name = os.path.join(path + '/event_frame', files[index][:-4]+'_10.png')
            imageio.imsave(save_name, event_image.transpose(1,2,0))

            events = events_slicer.get_events(t2, t2+time_delta)
            ...
            save_name = os.path.join(path + '/event_frame', files[index][:-4]+'_11.png')
            imageio.imsave(save_name, event_image.transpose(1,2,0))
            file.close()
            ...
            #     cv2.imwrite(os.path.join(os.path.join(path, 'disp   1'), files[index]), disp.astype(np.uint16))
