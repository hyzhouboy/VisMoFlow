...
from models import BaseModel, CamLiPWC, CamLiRAFT, PWC, CamLiPWC_L, RAFT, CamLiRAFT_L, VisMoFlow
from flyingthings3d import FlyingThings3D
from kitti import KITTI
from kitti_event import KITTI_Event
...
def dataset_factory(cfgs: DictConfig):
    if cfgs.name == 'flyingthings3d':
        ...
        return Sintel(cfgs)
    elif cfgs.name == 'kitti_event':
        return KITTI_Event(cfgs)
    elif cfgs.name == 'dsec':
        return DSEC(cfgs)
    else:
...
def model_factory(cfgs: DictConfig) -> BaseModel:
    ...
    elif cfgs.name == 'vismoflow':
        return VisMoFlow(cfgs)
    elif cfgs.name == 'pwc':
        return PWC(cfgs)
