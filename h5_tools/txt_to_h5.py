...
def extract_txt(txt_path, output_path, zero_timestamps=False,
    ...
    for i, event_window in enumerate(iterator):
        ...
        ts = (events[:, 0]*1e6).astype(np.int64)
        xs = events[:, 1].astype(np.int16)
        ys = events[:, 2].astype(np.int16)
        ps = events[:, 3]
        ps[ps < 0] = -1  # should be [-1 or 1]
        ps = ps.astype(bool)
...
if __name__ == "__main__":
    ...
    path = 'E:/4000datasets/self-record/20230220_light_dark/dark/recording_2023-02-20_20-14-42_20.0ms/test_davis346/outdoors_walking.zip'
    output_dir = 'E:/4000datasets/self-record/20230220_light_dark/dark/recording_2023-02-20_20-14-42_20.0ms/test_davis346'
    zero_timestamps = False

    print('Data will be extracted in folder: {}'.format(output_dir))
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    if os.path.isdir(path):
        txt_paths = sorted(list(glob.glob(os.path.join(path, "*.txt")))
                         + list(glob.glob(os.path.join(path, "*.zip"))))
    else:
        txt_paths = [path]
    extract_txts(txt_paths, output_dir, zero_timestamps=zero_timestamps)
