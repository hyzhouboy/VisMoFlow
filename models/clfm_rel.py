...
class CLFM_REL(nn.Module):
    def __init__(self, in_channels_2d, in_channels_3d, fusion_fn='sk', norm=None):
        ...
        if fusion_fn == 'add':
            ...
            # self.fuse_e2r = SKFusion(in_channels_2d, in_channels_2d, in_channels_2d, 'nchw', norm, reduction=2)
            # self.fuse_e2l = SKFusion(in_channels_2d, in_channels_3d, in_channels_3d, 'ncm', norm, reduction=2)
        else:
            raise ValueError

    @timer.timer_func
    def forward(self, uv, feat_2d, feat_3d, feat_ev2d):
        feat_2d = feat_2d.float()
        feat_3d = feat_3d.float()
        feat_ev2d = feat_ev2d.float()

        # event feature fusion
        # enhanced_feat_2d = self.fuse_e2r(feat_2d, feat_ev2d)
        feat_ev2d_sampled = grid_sample_wrapper(feat_ev2d.detach(), uv)
        enhanced_feat_3d = self.fuse3d(self.mlps3d(feat_ev2d_sampled.detach()), feat_3d)

        # 2d fusion
        feat_3d_interp = self.interp(uv, feat_2d.detach(), enhanced_feat_3d.detach())
        ...
        out3d = self.fuse3d(self.mlps3d(feat_2d_sampled.detach()), enhanced_feat_3d)
