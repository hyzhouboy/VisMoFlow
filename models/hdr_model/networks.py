...
def define_UpsampleNet(gpu_ids=[], scale=2):
    ...
    if len(gpu_ids) > 0:
        net.to(gpu_ids[0])    
        if scale == 2:
            net.load_state_dict(torch.load('checkpoint/upsample_net/upsample_net_2x.pth'))
        elif scale == 4:
            net.load_state_dict(torch.load('checkpoint/upsample_net/upsample_net_4x.pth'))
        elif scale == 8:
            net.load_state_dict(torch.load('checkpoint/upsample_net/upsample_net_8x.pth'))            
