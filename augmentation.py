...
def crop_image_with_pc(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, drop_pc=False, intensity_image=None, events=None, event_voxel=None):
    ...
    if events is not None:
        for i in range(len(events)):
            delete_id = np.vstack((np.argwhere(events[i][..., 1] > x2), np.argwhere(events[i][..., 1] < x1), np.argwhere(events[i][..., 2] > y2), np.argwhere(events[i][..., 2] < y1)))
            events[i][..., 0] = np.delete(events[i][..., 0], delete_id)
            events[i][..., 1] = np.delete(events[i][..., 1], delete_id)
            events[i][..., 2] = np.delete(events[i][..., 2], delete_id)
            events[i][..., 3] = np.delete(events[i][..., 3], delete_id)
    # crop intensity images
    if intensity_image is not None:
        for i in range(len(intensity_image)):
            intensity_image[i] = intensity_image[i][y1:y2, x1:x2].copy()
    # crop event_voxel
    if event_voxel is not None:
        for i in range(len(event_voxel)):
            event_voxel[i] = event_voxel[i][y1:y2, x1:x2].copy()
    ...
    return image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, intensity_image, events, event_voxel


def random_crop(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_size, drop_pc, intensity_image=None, events=None, event_voxel=None):
    assert flow_3d.shape[1] <= 4
    ...
    # 适用于DSEC： 10 20 20 40
    x1 = np.random.randint(low=20, high=image_w - 20 - crop_w + 1)
    y1 = np.random.randint(low=10, high=image_h - 40 - crop_h + 1)

    crop_window = [x1, y1, x1 + crop_w, y1 + crop_h]

    return crop_image_with_pc(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, drop_pc, intensity_image, events, event_voxel)
...
def random_scale(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, scale_range, intensity_image=None, events=None, event_voxel=None):
    ...
    image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, intensity_image, events, event_voxel = crop_image_with_pc(
        image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, crop_window, intensity_image, events, event_voxel
    )
...
def joint_augmentation(image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, cfgs, intensity_image=None, events=None, event_voxel=None):
    if not cfgs.enabled:
        return image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, intensity_image, events, event_voxel


    if events is not None or event_voxel is not None:  #有事件的时候，仅仅处理crop
        if cfgs.random_crop.enabled:
            image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy, intensity_image, events, event_voxel = random_crop(
                image1, image2, pc1, pc2, flow_2d, flow_3d, f, cx, cy,
                crop_size=cfgs.random_crop.crop_size,
                drop_pc=cfgs.random_crop.drop_pc,
                intensity_image=intensity_image, events=events, event_voxel=event_voxel
            )
    else:
